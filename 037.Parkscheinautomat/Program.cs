...
do
{ 
    Console.WriteLine($"Parktime until now: {PrintParkingTime()}");
...
bool EnterCoins(string input, int counter)
{
    if (input == "d" || input == "D")
        if (counter == 1)
        {
            Console.WriteLine("Minimum  50 cent");
            return false;
        }
        else
        {
            Console.WriteLine($"You are allowed to to park here for {PrintParkingTime()}hours");
            return true;
        }
    else
    {
...
string PrintParkingTime()
{
    int hours = time / 60;
    if (hours > 1)
        return $"You are allowed to park here for 1.30 hours";
    else
...
void PrintDonation()
{
    ...
    Console.WriteLine($"Thank you for{donation:f2} euros");
}
